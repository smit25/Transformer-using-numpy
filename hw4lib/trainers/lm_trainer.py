...
class LMTrainer(BaseTrainer):
    ...
    def __init__(self, model, tokenizer, config, run_name, config_file, device=None):
        ...
        self.criterion = nn.CrossEntropyLoss(label_smoothing=self.config['loss'].get('label_smoothing', 0.0),
                                             ignore_index=self.tokenizer.pad_id if hasattr(self.tokenizer, 'pad_id') else -100)
    ...
    def train(self, train_dataloader, val_dataloader, epochs: int):
        ...
        for epoch in range(self.current_epoch, self.current_epoch + epochs):
            ...
            gen_results = self.generate(val_dataloader,generation_config=self._get_evaluation_generation_configs().get('greedy', None))
    ...
    def generate(self, dataloader, generation_config: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        with torch.inference_mode():
            if generation_config.get('top_k', 0) > 0 or generation_config.get('top_p', 0) > 0:
                ...
                seqs, scores = generator.generate_sample(prompts,generation_config.get('temperature', 1.0), top_k=generation_config.get('top_k', 0),top_p=generation_config.get('top_p', 0.0))
                ...
                seqs, scores = generator.generate_beam(prompts,beam_width=generation_config.get('beam_width', 10),temperature=generation_config.get('temperature', 1.0),repeat_penalty=generation_config.get('repeat_penalty', 1.2))
                ...
                seqs, scores = generator.generate_greedy(prompts,generation_config.get('temperature', 1.0),repeat_penalty=generation_config.get('repeat_penalty', 1.2))
