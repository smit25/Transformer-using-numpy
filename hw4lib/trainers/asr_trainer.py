...
class ASRTrainer(BaseTrainer):
    ...
    def _train_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            feat_lengths = torch.tensor(feat_lengths).to(self.device)
            transcript_lengths = torch.tensor(transcript_lengths).to(self.device)
            ...
            with torch.autocast(device_type=self.device, dtype=torch.float16):
                ...
                if self.ctc_weight > 0:
                    ctc_loss = self.ctc_criterion(ctc_inputs["log_probs"], targets_golden, feat_lengths, transcript_lengths)
    ...
    def _validate_epoch(self, dataloader):
        ...
        # TODO: Call recognize
        results = self.recognize(dataloader, recognition_config=None, config_name='validation')
        
        # TODO: Extract references and hypotheses from results
        references = [result["target"] for result in results if "target" in result]
        hypotheses = [result["generated"] for result in results if "generated" in result]
