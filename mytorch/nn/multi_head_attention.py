...
class MultiHeadAttention:
    ...
    def _merge_masks(self, key_padding_mask, attn_mask):
        ...
        key_mask = np.broadcast_to(key_mask, (key_padding_mask.shape[0], self.num_heads, attn_mask.shape[0], key_padding_mask.shape[-1]))
        
        # Expand attn_mask to (1, 1, L, S) and broadcast to (N, H, L, S)
        attention_mask = np.expand_dims(attn_mask, axis = (0,1))
        attention_mask = np.broadcast_to(attention_mask, (key_padding_mask.shape[0], self.num_heads, attn_mask.shape[0], key_padding_mask.shape[-1]))
